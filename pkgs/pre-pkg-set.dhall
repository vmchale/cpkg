...
let gdk-pixbuf =
  λ(homedir : types.BuildVars → Text) →
  let gdkInstall =
    ...
    λ(cfg : types.BuildVars) →
      ...
      , prelude.call (prelude.defaultCall ⫽ { program = "ninja"
                                            , environment = Some [ prelude.mkPkgConfigVar cfg.linkDirs
                                                                ...
                                                                -- FIXME: causes problems with cpkg list
                                                                 , { var = "PATH", value = prelude.mkPathVar cfg.binDirs ++ ":${homedir cfg}/.cpkg/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin" }
...
λ(homedir : types.BuildVars → Text) →
  ...
  , gc [8,0,2]
  , gdb [8,2]
  , gdk-pixbuf (λ(_ : types.BuildVars) → "/home/vanessa") { version = [2,38], patch = 0 }
  , gettext [0,19,8]
  , gperf [3,1]
  , giflib [5,1,4]
  , git [2,19,2]
